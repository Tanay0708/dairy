...
const Hero = () => {
  ...
  return (
    <section className=" w-full h-screen flex max-sm:flex-col max-sm:mt-24   ">
      {/* text div */}
      ...
      {/* image div */}
      <div className=" w-[50%] h-[full] max-sm:w-full max-sm:h-[100%] relative  max-sm:ml-5  ">
          <img src={dairy01}  className="z-10  max-lg:h-[40%] h-[80%] w-full  absolute -top-8 right-6 max-sm:top-10 max-sm:right-10 max-sm:w-full max-sm:h-[55%]  "  alt="" />
          <img src={line}  className= "z-0 absolute -top-60 -left-80   -rotate-[70deg] max-sm:hidden h-[800px] max-lg:hidden" alt="" />
          <img src={plane} className="absolute w-24 -top-0 -rotate-[70deg] -left-40 max-sm:left-52 max-sm:w-12 max-sm:-top-52 max-lg:hidden max-sm:block z-0" alt="" />
      </div>
